...
def test_add_user(test_app, test_database):
    ...
    assert "michael@example.io was added!" in data["message"]
...
def test_add_user_invalid_json_keys(test_app, test_database):
    ...
    resp = client.post(
        "/users",
        data=json.dumps({"email": "john@example.io"}),
        content_type="application/json",
    )
    data = json.loads(resp.data.decode())
    assert resp.status_code == 400
...
def test_single_user(test_app, test_database, add_user):
    user = add_user("jeffrey", "jeffrey@example.io", "greaterthaneight")
    client = test_app.test_client()
    resp = client.get(f"/users/{user.id}")
    data = json.loads(resp.data.decode())
    assert resp.status_code == 200
    assert "jeffrey" in data["username"]
    assert "jeffrey@example.io" in data["email"]
...
def test_remove_user(test_app, test_database, add_user):
    test_database.session.query(User).delete()
    user = add_user("user-to-be-removed", "remove-me@example.io", "greaterthaneight")
    client = test_app.test_client()
    ...
    assert "remove-me@example.io was removed!" in data["message"]
...
def test_update_user(test_app, test_database, add_user):
    user = add_user("user-to-be-updated", "update-me@example.io", "greaterthaneight")
    client = test_app.test_client()
    resp_one = client.put(
        f"/users/{user.id}",
        data=json.dumps({"username": "me", "email": "me@example.io"}),
        content_type="application/json",
    )
    data = json.loads(resp_one.data.decode())
    assert resp_one.status_code == 200
    ...
    assert "me@example.io" in data["email"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [1, {"email": "me@example.io"}, 400, "Input payload validation failed"],
        [
            999,
            {"username": "me", "email": "me@example.io"},
            404,
...
def test_update_user_with_passord(test_app, test_database, add_user):
    ...
    user = add_user("user-to-be-updated", "update-me@example.io", password_one)
    ...
    resp = client.put(
        f"/users/{user.id}",
        data=json.dumps(
            {"username": "me", "email": "me@example.io", "password": password_two}
        ),
        content_type="application/json",
    )
    assert resp.status_code == 200
